...
* `selmerGroup₂_eq_badPrimes` (over a number field): the local conditions at the good finite
  ...
  `Heights.ForMathlib.WeierstrassFormalGroup.Filtration`.
* `card_range_μ_adicCompletion`, `card_range_μ_completion_isReal`,
  ...
  `(𝒪_v, +)`, proved in `Heights.ForMathlib.WeierstrassFormalGroup.Filtration`).
...
The formal-group input (proved in `Heights.ForMathlib.WeierstrassFormalGroup.Filtration`) is
