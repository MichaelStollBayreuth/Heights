...
The ring `𝒪_S` is now known to be a Dedekind domain (`isDedekindDomain`). What remains is the
description of its arithmetic in terms of `R`:

* `heightOneSpectrumEquiv`: `Ideal.map` and `Ideal.comap` are mutually inverse between the primes
  of `R` off `S` (`primeOfNotMem`) and the primes of `𝒪_S`. `map_comap_eq` gives one composite;
  for the other, the contraction of a prime of `𝒪_S` is a nonzero prime of `R` (`comap_ne_bot`)
  not in `S` (as `map_asIdeal_eq_top` kills the primes of `S`).
* `valuation_heightOneSpectrumEquiv`: the `v`-adic valuation of `𝒪_S` is the restriction of that
  of `R`, so the Selmer group of `𝒪_S` relative to `∅` is the Selmer group of `R` relative to
  `S`.
