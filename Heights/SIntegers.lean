...
The ring `𝒪_S` is now known to be a Dedekind domain (`isDedekindDomain`). Its height one spectrum is `{v ∉ S}` (`heightOneSpectrumEquiv`).  What remains is the
compatibility of valuations and the class group:
